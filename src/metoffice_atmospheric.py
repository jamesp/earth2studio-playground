"""Native Met Office atmospheric data from Microsoft Planetary Computer.

This module provides :class:`MetOfficePlanetaryComputer`, a DataSource
returning raw Met Office fields on the native ~0.09° grid with native
variable names.  No derived variables, no regridding; longitude is
normalized to [0, 360] via xarray coordinate operations.
...
Usage::
    ...
    ds = MetOfficePlanetaryComputer()
    da = ds(["2026-02-17T00:00"], ["air_temperature_500hPa", "wind_speed_at_10m"])
"""
...
def _build_vocab() -> dict[str, tuple[str, Modifier]]:
    """Build the vocabulary mapping native Met Office names to STAC asset locations.
...
class MetOfficeAtmosphericLexicon(metaclass=LexiconType):
...
_NATIVE_NLAT: int = 1920
_NATIVE_NLON: int = 2560

_RAW_LON_COORDS = np.linspace(
    -179.9296875, 179.9296875, _NATIVE_NLON, dtype=np.float32
)


class MetOfficePlanetaryComputer(_PlanetaryComputerData):
    ...
    >>> from src.metoffice_atmospheric import MetOfficePlanetaryComputer
    >>> ds = MetOfficePlanetaryComputer()
    >>> da = ds(["2026-02-17T00:00"], ["air_temperature_500hPa"])
    """
    ...
    LAT_COORDS = np.linspace(-89.953125, 89.953125, _NATIVE_NLAT, dtype=np.float32)
    LON_COORDS = np.sort(_RAW_LON_COORDS % 360)
    ...
    def __init__(
        ...
        super().__init__(
            ...
            lexicon=MetOfficeAtmosphericLexicon,
            asset_key="temperature_on_pressure_levels",  # nominal; overridden per-variable
    ...
    def _validate_time(times: list[datetime]) -> None:
        ...
        for t in times:
            if (
                t.hour not in MetOfficePlanetaryComputer._VALID_RUN_HOURS
                or t.minute
                or t.second
            ):
                raise ValueError(
    ...
    def _prepare_asset_plans(
        ...
        for spec in variables:
            ...
            needed_assets.setdefault((col, asset_key), []).append(spec)
    ...
    def _spec_to_asset_key(spec: VariableSpec) -> tuple[str, str]:
        ...
        asset_key = parts[1]
        col = (
            MetOfficePlanetaryComputer.PRESSURE_COLLECTION
            if collection_type == "pressure"
            else MetOfficePlanetaryComputer.SURFACE_COLLECTION
        )
    ...
    def _read_field(
        ...
        field = field.squeeze()
        # Normalize lon from native [-180, 180] to [0, 360] using xarray coordinate
        # operations, making the convention explicit in the DataArray rather than
        # hidden in a raw numpy roll.
